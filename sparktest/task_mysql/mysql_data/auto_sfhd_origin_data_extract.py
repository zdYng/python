...
import random
import subprocess
import sfhd_origin_data_extract as sd
import helper
import settings
...
class task_item(object):
    ...
    def process(self):
        t1 = utils.getTimeSec("2017-11-18 10:40:00")
        t2 = utils.getTimeSec("2016-07-04 00:00:00")
        now_time = int(time.time()-(t1-t2))
        start_time = utils.getTimeSec(utils.getDigitDay(now_time - 86400 * 3))
        end_time = utils.getTimeSec(utils.getDigitDay(now_time-86400))
        print(now_time,start_time,end_time)
        sd.time_main(start_time,end_time, settings.tablename)
